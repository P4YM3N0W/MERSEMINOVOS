...
const CACHE_NAME  = 'mer_seminovos_docs';
...
// ══ CACHE API (documentos) ════════════════════════════════════
async function salvarDocCache(chave, file) {
  try {
    const cache = await caches.open(CACHE_NAME);
    const resp  = new Response(file, { headers: { 'Content-Type': file.type } });
...
async function obterDocCache(chave) {
  try {
    const cache = await caches.open(CACHE_NAME);
...
async function deletarDocCache(chave) {
  try {
    const cache = await caches.open(CACHE_NAME);
...
function renderAlertas() {
  ...
  if (!alertas.length) {
    el.innerHTML = `<div class="empty-state" style="padding:24px">
      <p>Nenhum vencimento proximo nos proximos 60 dias</p></div>`;
    return;
  }
  ...
  el.innerHTML = alertas.slice(0, 7).map(a => {
    ...
    const txt = a.dias < 0
      ? `Vencido ha ${Math.abs(a.dias)}d`
      : a.dias === 0 ? 'Vence hoje' : `${a.dias}d restantes`;
    return `
...
function renderUltimos() {
  ...
  if (!sorted.length) {
    el.innerHTML = `<div class="empty-state" style="padding:24px">
      <p>Nenhum veiculo cadastrado ainda</p></div>`;
    return;
  }
  el.innerHTML = sorted.slice(0, 6).map(v => `
    <div class="alerta-item" onclick="openDetalhe('${v.id}')">
      <div style="width:34px;height:34px;background:rgba(255,179,0,.1);
        border:1px solid rgba(255,179,0,.2);border-radius:8px;
        display:flex;align-items:center;justify-content:center;
        color:#ffb300;flex-shrink:0">${svgCar(16)}</div>
      <div class="alerta-info">
...
function renderVeiculos() {
  ...
  listMode ? grid.classList.add('list-view') : grid.classList.remove('list-view');
  ...
  grid.innerHTML = filtered.map((v, i) => {
    ...
    return `
      <div class="veiculo-card" style="animation-delay:${i * .05}s"
           onclick="openDetalhe('${v.id}')">
        ...
        <div class="veiculo-footer">
          ...
          <div class="veiculo-actions" onclick="event.stopPropagation()">
            <button class="icon-btn btn-sm" title="Editar"
              onclick="openModal('${v.id}')">${svgEdit()}</button>
            <button class="icon-btn btn-sm" title="Deletar"
              onclick="deletarVeiculo('${v.id}')">${svgTrash()}</button>
          </div>
        </div>
      </div>`;
  }).join('');
...
function renderVencimentos() {
  ...
  if (!rows.length) {
    el.innerHTML = `<div class="empty-state">
      ${svgCal(48)}
      <h3>Nenhuma data cadastrada</h3>
      <p>Adicione datas nos veiculos para acompanhar aqui</p>
    </div>`;
    return;
  }
  ...
  el.innerHTML = rows.map((r, i) => {
    ...
    const txt = r.dias < 0
      ? `Vencido ha ${Math.abs(r.dias)} dias`
      : r.dias === 0 ? 'Vence hoje' : `${r.dias} dias restantes`;
    return `
      <div class="venc-row ${cls}" style="animation-delay:${i * .04}s"
           onclick="openDetalhe('${r.v.id}')">
        <div>
...
function renderDocumentos() {
  ...
  if (!comDocs.length) {
    el.innerHTML = `<div class="empty-state">
      ${svgDoc(48)}
      <h3>Nenhum documento salvo</h3>
      <p>Abra um veiculo e adicione documentos nos detalhes</p>
    </div>`;
    return;
  }
          ...
          <div class="doc-item">
            ...
            <div style="display:flex;gap:6px">
              <button class="btn btn-ghost btn-sm"
                onclick="verDoc('${d.cacheKey}')">Ver</button>
              <button class="btn btn-danger btn-sm"
                onclick="deletarDoc('${v.id}','${d.id}')">X</button>
            </div>
          </div>`).join('')}
      </div>
    </div>`).join('');
...
function saveVeiculo(e) {
  ...
  if (editId) {
    const idx = veiculos.findIndex(x => x.id === editId);
    if (idx !== -1) {
      veiculos[idx] = {
        ...veiculos[idx],
        ...dados,
...
async function deletarVeiculo(id) {
  ...
  // Remove docs do cache
  for (const d of (v.documentos || [])) {
    await deletarDocCache(d.cacheKey);
  }
...
function openDetalhe(id) {
  ...
  document.getElementById('detalheBody').innerHTML = `
    <div class="detalhe-grid">
      <div class="detalhe-info-group">
        <div class="detalhe-item">
          <label>Placa</label>
          <span class="veiculo-plate" style="font-size:14px">${v.placa || '—'}</span>
        </div>
        <div class="detalhe-item"><label>Tipo</label><span>${v.tipo || '—'}</span></div>
        <div class="detalhe-item">
          <label>Marca / Modelo</label>
          <span>${[v.marca, v.modelo, v.ano].filter(Boolean).join(' ') || '—'}</span>
        </div>
        ...
        <div class="detalhe-item">
          <label>Chassi</label>
          <span style="font-size:12px">${v.chassi || '—'}</span>
        </div>
      </div>
      <div class="detalhe-info-group">
        <div class="detalhe-item">
          <label>Proprietario</label><span>${v.proprietario || '—'}</span>
        </div>
        <div class="detalhe-item">
          <label>Status</label>
          <span class="status-badge status-${v.status || 'Regular'}">${v.status || 'Regular'}</span>
        ...
        <div class="detalhe-item">
          ...
          <span>${v.criadoEm ? formatDate(new Date(v.criadoEm)) : '—'}</span>
        </div>
        ${v.obs ? `<div class="detalhe-item">
          <label>Observacoes</label>
          <span style="font-size:13px;color:var(--text-sub)">${v.obs}</span>
        </div>` : ''}
      </div>
    </div>
    ...
    <div class="docs-section">
      ...
      <form onsubmit="uploadDoc(event,'${v.id}')">
        <div style="display:grid;grid-template-columns:1fr 130px 160px auto;
          ...
          <div class="form-group">
            ...
            <select id="uploadTipo">
              <option>CRLV</option><option>IPVA</option>
              <option>Seguro</option><option>CNH</option>
              <option>Laudo</option><option>Outro</option>
            </select>
          </div>
          <div class="form-group">
            <label>Arquivo</label>
...
async function uploadDoc(e, veiculoId) {
  ...
  const cacheKey = `doc_${Date.now()}_${file.name}`;
  const salvo    = await salvarDocCache(cacheKey, file);
  ...
  const idx = veiculos.findIndex(x => x.id === veiculoId);
  if (idx === -1) return;

  veiculos[idx].documentos.push({
    id:        crypto.randomUUID(),
    nome:      nome || file.name,
    tipo,
    cacheKey,
  ...
  veiculos[idx].atualizadoEm = new Date().toISOString();
  salvarLocal();
...
// ══ VER DOCUMENTO ════════════════════════════════════════════
async function verDoc(cacheKey) {
  const url = await obterDocCache(cacheKey);
  if (!url) {
    toast('Documento nao encontrado no cache', 'error');
    return;
  }
...
async function deletarDoc(veiculoId, docId) {
  ...
  const doc = veiculos[idx].documentos.find(d => d.id === docId);
  if (doc) await deletarDocCache(doc.cacheKey);

  veiculos[idx].documentos     = veiculos[idx].documentos.filter(d => d.id !== docId);
  veiculos[idx].atualizadoEm   = new Date().toISOString();
  salvarLocal();
...
function exportarXLS() {
  try {
    if (!veiculos.length) { toast('Nenhum veiculo para exportar', 'warn'); return; }
    ...
    const linhas = veiculos.map(v => ({
      ...
      'Cadastrado em':       v.criadoEm
        ? new Date(v.criadoEm).toLocaleDateString('pt-BR') : ''
    }));

    const ws = XLSX.utils.json_to_sheet(linhas);
    ws['!cols'] = [
      {wch:22},{wch:12},{wch:12},{wch:12},{wch:16},{wch:16},{wch:8},
      {wch:12},{wch:16},{wch:24},{wch:22},{wch:28},{wch:16},{wch:20},
      {wch:14},{wch:12},{wch:16},{wch:12},{wch:14},{wch:16}
    ];

    const wb   = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Veiculos');
    XLSX.writeFile(wb, `MeR_Seminovos_${new Date().toISOString().slice(0,10)}.xlsx`);
...
async function importarArquivo(file) {
  try {
    ...
    if (ext === 'json') {
      ...
      arr = Array.isArray(data)
        ...
        : Array.isArray(data.veiculos)
          ? data.veiculos : [];
      if (!arr.length) throw new Error('Nenhum dado no JSON');
    }
    ...
    if (!arr.length) throw new Error('Nenhum dado encontrado no arquivo');
    ...
    // Substitui tudo no localStorage
    veiculos = arr;
    salvarLocal();
    renderView(currentView);
...
function toast(msg, tipo = 'success') {
  ...
  el.innerHTML = `
    <div style="width:8px;height:8px;border-radius:50%;
      background:${cores[tipo] || cores.success};flex-shrink:0"></div>
    <span>${msg}</span>`;
  wrap.appendChild(el);
...
function svgCar(s = 20) {
  return `<svg width="${s}" height="${s}" fill="none" stroke="currentColor"
